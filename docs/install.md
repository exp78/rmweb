...
archives are attached to [GitHub Releases](https://github.com/exp78/rmweb/releases) — install v0.9.2
without any toolchain:

```sh
# 1. Download the archive from Releases and copy it to the tablet over USB:
scp rmweb-0.9.2.tar.gz root@10.11.99.1:/home/root/
# 2. Extract + wire up on the device (no toolchain here):
ssh root@10.11.99.1 'mkdir -p /home/root/rmweb \
  && gunzip -c /home/root/rmweb-0.9.2.tar.gz | tar -C /home/root/rmweb -xf - \
...
`./scripts/build-wpeqt.sh && ./scripts/package.sh`, then the same two steps with `dist/rmweb-0.9.2.tar.gz`.
