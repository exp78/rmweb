...
int main() {
    ...
    // settings: block / siteCss / bwFast / textBoost / settleFlash / autoRefreshSec round-trip + defaults
    Settings sb; sb.block = false; sb.siteCss = false; sb.bwFast = true; sb.textBoost = false; sb.settleFlash = false; sb.autoRefreshSec = 60;
    CHECK(saveSettings(dir, sb));
    Settings sb2 = loadSettings(dir);
    CHECK(!sb2.block); CHECK(!sb2.siteCss); CHECK(sb2.bwFast); CHECK(!sb2.textBoost); CHECK(!sb2.settleFlash);
    CHECK(sb2.autoRefreshSec == 60);
    CHECK(detail::atomicWrite(dir + "/settings.txt", "zoom=1.0\n"));
    ...
    CHECK(sb3.settleFlash);                                       // settleFlash defaults ON
