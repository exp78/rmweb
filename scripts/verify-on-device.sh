...
# Prereqs: device on USB (10.11.99.1), ./scripts/build-wpeqt.sh run. Safe to re-run; ~3 min.
cd "$(dirname "$0")/.."
[ -f .env ] && . ./.env || true
...
echo "== [0/8] waiting for the device =="
up=0
...
EOF
  ...
  echo "== [6/8] password store: learn, then prefill =="
  fresh
