...
// Optional mobile User-Agent (opt-in via RMWEB_UA=mobile): makes heavy JS-app sites (e.g. ixbt) serve their
...
public:
    explicit WpeView(QQuickItem *parent = nullptr) : QQuickPaintedItem(parent) {
        ...
        // Content present throttle: heavy SPAs (rbc.ru etc.) emit NEW frames every ~300ms forever
