...
struct Settings { double zoom = 1.0; int readerFont = 30; std::string ua; bool readerDark = false;
                  ...
                  bool settleFlash = true;  // one full-quality panel develop once the page goes quiet
                                            // (the QPA auto waveform underdrives black on fast presents)
...
// Works for any entry type with .title/.url (HistoryEntry, Bookmark).
template<typename T>
inline std::vector<T> searchStore(const std::vector<T>& v, const std::string& q, size_t cap = 20) {
    std::vector<T> out;
    for (const auto& e : v) {
...
inline bool saveSettings(const std::string& dir, const Settings& s) {
    std::string out = "zoom=" + std::to_string(s.zoom) + "\n"
                    ...
                    + "settleFlash=" + std::string(s.settleFlash ? "1" : "0") + "\n"
